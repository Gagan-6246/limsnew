...
const Order = () => {
    ...
    async function openEditPopup(data, r) {
        ...
        let tableData = rowData[0].orderList.map((t, index) => {
            return {
    ...
    const deleteFileConfirm = async () => {
        let result = await deleteOrderFile(deleteFileData);
        if (result) {
    ...
    async function deleteDataMain(data) {
        let result = await deleteOrder(data);
        console.log(result);
        // setAddAnalysisServiceData(data);
    ...
    const onSubmit = async (formData) => {
        ...
        if (formData.file.length != 0) {
            let result = await updateOrderFile(formData);
            console.log(result);
        } else {
            ...
            let result = await updateOrder(formData);
            console.log(result);
        }
        ...
        <>
            ...
            <Row>
                <Col>
                    <Card>
                        <Card.Body>
                            <Button className="float-end" onClick={() => navigate('/apps/settings/addOrders')}>
